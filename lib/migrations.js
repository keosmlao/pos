...
export async function ensurePurchaseRequestsSchema() {
  ...
  try {
    ...
    console.error('purchase_requests migration error:', e.message); throw e;
  }
}
...
export async function ensureStockTransfersSchema() {
  ...
  try {
    ...
    console.error('stock_transfers migration error:', e.message); throw e;
  }
}
...
export async function ensureLaybysSchema() {
  ...
  try {
    ...
    `);
    await pool.query(`ALTER TABLE laybys ADD COLUMN IF NOT EXISTS credit_used NUMERIC DEFAULT 0`);
    ...
    console.error('laybys migration error:', e.message); throw e;
  }
}
...
export async function ensureStockTakesSchema() {
  ...
  try {
    ...
    console.error('stock_takes migration error:', e.message); throw e;
  }
}
...
export async function ensureStockAdjustmentsSchema() {
  ...
  try {
    ...
    // 1 ໃບ (adjustment_number) ມີໄດ້ຫຼາຍແຖວ — ແຖວລະສິນຄ້າ
    // ເມື່ອກ່ອນເປັນ UNIQUE ຈຶ່ງສ້າງໃບທີ່ມີສິນຄ້າຫຼາຍກວ່າ 1 ລາຍການບໍ່ໄດ້
    // ຄວາມບໍ່ຊ້ຳຂອງເລກໃບຮັບປະກັນໂດຍ allocateDocumentNumber() ຢູ່ແລ້ວ
    await pool.query(`DROP INDEX IF EXISTS idx_stock_adj_number`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_stock_adj_number ON stock_adjustments(adjustment_number) WHERE adjustment_number IS NOT NULL`);
    ...
    console.error('stock_adjustments migration error:', e.message); throw e;
  }
}
...
export async function ensureParkedCartsSchema() {
  ...
  try {
    ...
    console.error('parked_carts migration error:', e.message); throw e;
  }
}
...
export async function ensureBranchesSchema() {
  ...
  try {
    ...
    console.error('branches migration error:', e.message); throw e;
  }
}
...
export async function ensureProductVariantsSchema() {
  ...
  try {
    ...
    console.error('product_variants migration error:', e.message); throw e;
  }
}
...
export async function ensureAuditLogsSchema() {
  ...
  try {
    ...
    console.error('audit_logs migration error:', e.message); throw e;
  }
}
...
export async function ensureQuotationsSchema() {
  ...
  try {
    ...
    console.error('quotations migration error:', e.message); throw e;
  }
}
...
export async function ensureCashTransactionsSchema() {
  ...
  try {
    ...
    console.error('cash_transactions migration error:', e.message); throw e;
  }
}
...
export async function ensureProductsExtraSchema() {
  ...
  try {
    ...
    console.error('products extra migration error:', e.message); throw e;
  }
  // ກັນລະຫັດ/ບາໂຄດຊ້ຳ — ຖ້າຍັງມີຂໍ້ມູນຊ້ຳຄ້າງຢູ່ index ຈະສ້າງບໍ່ສຳເລັດ (ແຈ້ງໃນ log) ແຕ່ບໍ່ກະທົບການໃຊ້ງານ
  try {
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_products_code_unique
      ON products (product_code) WHERE product_code IS NOT NULL AND product_code <> ''`);
  } catch (e) {
    console.error('products unique code index:', e.message);
  }
  try {
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_products_barcode_unique
      ON products (barcode) WHERE barcode IS NOT NULL AND barcode <> ''`);
  } catch (e) {
    console.error('products unique barcode index:', e.message);
  }
}
...
export async function ensureUsersSchema() {
  ...
  try {
    ...
    // users API ຂຽນ branch_id ຢູ່ແລ້ວ ແຕ່ຄໍລຳຫາຍ → ສ້າງ user ໃໝ່ບໍ່ໄດ້ໃນ DB ໃໝ່
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS branch_id INTEGER`);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_username ON users(username)`);
    usersMigrated = true;
  } catch (e) {
    console.error('users migration error:', e.message); throw e;
  }
}
...
export async function ensureOrdersSchema() {
  ...
  try {
    ...
    // ສະຖານະສົ່ງຂໍ້ມູນບິນອາກອນ — NULL = ຍັງບໍ່ໄດ້ສົ່ງ (ຄ່າເລີ່ມຕົ້ນ)
    await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS tax_submitted_at TIMESTAMP`);
    ...
    // ຕົ້ນທຶນ ณ ເວລາຂາຍ (snapshot) — ໃຊ້ໃນລາຍງານກຳໄລ; ບິນເກົ່າທີ່ NULL ຈະ fallback ໃສ່ຕົ້ນທຶນປັດຈຸບັນ
    await pool.query(`ALTER TABLE order_items ADD COLUMN IF NOT EXISTS cost_price NUMERIC`);
    ordersMigrated = true;
  } catch (e) {
    console.error('orders migration error:', e.message); throw e;
  }
}
...
export async function ensureMembersSchema() {
  ...
  try {
    ...
    // ເລກປະຈຳຕົວຜູ້ເສຍອາກອນ (TIN) — ເກັບເປັນ TEXT ບໍ່ແມ່ນຕົວເລກ
    // ເພາະຕ້ອງຮັກສາເລກ 0 ຂ້າງໜ້າ ແລະ ຄວາມຍາວ 12 ຕົວໃຫ້ຄົງທີ່
    await pool.query(`ALTER TABLE members ADD COLUMN IF NOT EXISTS tax_id TEXT`);
    ...
    console.error('members migration error:', e.message); throw e;
  }
}
...
export async function ensureCustomerDebtPaymentsSchema() {
  ...
  try {
    ...
    console.error('customer_debt_payments migration error:', e.message); throw e;
  }
}
...
export async function ensureReturnsSchema() {
  ...
  try {
    ...
    `);
    // ຄືນສິນຄ້າ → ຫັກແຕ້ມທີ່ໄດ້ຮັບຄືນ ແລະ ຄືນແຕ້ມທີ່ໃຊ້ໄປ (ເກັບໄວ້ເພື່ອຍ້ອນກັບໄດ້)
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS member_id INTEGER`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS member_points_reverted INTEGER NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS member_points_restored INTEGER NOT NULL DEFAULT 0`);
    // ມູນຄ່າສິນຄ້າຕາມລາຄາປ້າຍ (gross) ແລະ ສ່ວນຫຼຸດທີ່ຖືກປັນສ່ວນມາຈາກບິນຂາຍ —
    // refund_amount ຄືເງິນທີ່ຄືນຈິງ = gross − ສ່ວນຫຼຸດຕາມບິນ − ຫັກເພີ່ມ
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS gross_amount NUMERIC NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS discount_amount NUMERIC NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS extra_deduction NUMERIC NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE return_items ADD COLUMN IF NOT EXISTS net_price NUMERIC`);
    // ປັດເສດຍອດຄືນເງິນ (ບໍ່ໃຫ້ເປັນຈຳນວນເສດ) — ເກັບຄ່າທີ່ໃຊ້ໄວ້ເພື່ອກວດຍ້ອນຫຼັງ
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS rounding_adjustment NUMERIC NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS rounding_step INTEGER NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE returns ADD COLUMN IF NOT EXISTS rounding_mode TEXT`);
    // ໃບຄືນເກົ່າ (ກ່ອນມີການປັນສ່ວນສ່ວນຫຼຸດ) ຄືນເງິນເປັນລາຄາປ້າຍເຕັມ → gross = refund
    await pool.query(`UPDATE returns SET gross_amount = refund_amount WHERE gross_amount = 0 AND refund_amount > 0`);
    ...
    console.error('returns migration error:', e.message); throw e;
  }
}

let purchaseReturnsMigrated = false;
// ສົ່ງເຄື່ອງຄືນໃຫ້ຜູ້ສະໜອງ (credit note / debit note ຝັ່ງຊື້)
export async function ensurePurchaseReturnsSchema() {
  if (purchaseReturnsMigrated) return;
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS purchase_returns (
        ...
        refund_amount NUMERIC NOT NULL DEFAULT 0,
        settle_mode TEXT NOT NULL DEFAULT 'refund',
        payments JSONB,
        note TEXT,
        created_by TEXT,
        created_at TIMESTAMP DEFAULT NOW()
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_purchase_returns_purchase ON purchase_returns(purchase_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_purchase_returns_created ON purchase_returns(created_at DESC)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_purchase_return_items_item ON purchase_return_items(purchase_item_id)`);
    purchaseReturnsMigrated = true;
  } catch (e) {
    console.error('purchase_returns migration error:', e.message); throw e;
  }
}
...
export async function ensureCompanyProfileSchema() {
  ...
  try {
    ...
    // ຊ່ວງນັບສະສົມແຕ້ມ (ວ່າງ = ບໍ່ຈຳກັດ) ແລະ ວັນສຸດທ້າຍທີ່ໃຊ້ແຕ້ມໄດ້
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS points_earn_start DATE`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS points_earn_end DATE`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS points_redeem_deadline DATE`);
    ...
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS purchase_return_number_template TEXT DEFAULT '{{prefix}}-{{YYYY}}{{MM}}-{{seq}}'`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS purchase_return_number_prefix TEXT DEFAULT 'PRT'`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS purchase_return_number_seq_digits INTEGER DEFAULT 4`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS purchase_return_number_seq_reset TEXT DEFAULT 'monthly'`);
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS purchase_return_number_start INTEGER DEFAULT 1`);
    ...
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS payment_qr_url TEXT`);
    // ປຸ່ມ "ພິມບິນ ອມພ" ໃນປະຫວັດການຂາຍ POS — ເປີດ/ປິດຈາກໜ້າຫຼັງບ້ານ (ຂໍ້ມູນບໍລິສັດ)
    await pool.query(`ALTER TABLE company_profile ADD COLUMN IF NOT EXISTS vat_bill_button_enabled BOOLEAN DEFAULT FALSE`);
    await pool.query(`
    ...
    console.error('company_profile migration error:', e.message); throw e;
  }
}
...
export async function ensurePendingInvoicesSchema() {
  ...
  try {
    ...
    // ເຊື່ອງບິນທີ່ບໍ່ຕ້ອງການ — ຕ້ອງເກັບແຖວໄວ້ (ບໍ່ລຶບ) ບໍ່ດັ່ງນັ້ນ sync ຮອບຕໍ່ໄປຈະດຶງກັບມາອີກ
    await pool.query(`ALTER TABLE pending_invoices ADD COLUMN IF NOT EXISTS dismissed_at TIMESTAMP`);
    await pool.query(`ALTER TABLE pending_invoices ADD COLUMN IF NOT EXISTS dismiss_reason TEXT`);
    await pool.query(`ALTER TABLE pending_invoices ADD COLUMN IF NOT EXISTS dismissed_by TEXT`);
    pendingInvoicesMigrated = true;
  } catch (e) {
    console.error('pending_invoices migration error:', e.message); throw e;
  }
}
...
export async function ensurePriceHistorySchema() {
  ...
  try {
    ...
    console.error('price_history migration error:', e.message); throw e;
  }
}
...
export async function ensurePromotionsSchema() {
  ...
  try {
    ...
    console.error('promotions migration error:', e.message); throw e;
  }
}
...
export async function ensureCurrenciesSchema() {
  ...
  try {
    ...
    console.error('currencies migration error:', e.message); throw e;
  }
}
...
export async function ensureCashHandoversSchema() {
  ...
  try {
    ...
    console.error('cash_handovers migration error:', e.message); throw e;
  }
}

export async function ensureAppEventsSchema() {
  if (appEventsMigrated) return;
  try {
    await pool.query(`
    ...
    `);
    await pool.query(`ALTER TABLE app_events ADD COLUMN IF NOT EXISTS read_at TIMESTAMP`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_app_events_created ON app_events(created_at DESC)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_app_events_type ON app_events(type)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_app_events_unread ON app_events(read_at) WHERE read_at IS NULL`);
    appEventsMigrated = true;
  } catch (e) {
    console.error('app_events migration error:', e.message); throw e;
  }
}
