...
export const PUT = handle(async (request, { params }) => {
  ...
  const result = await pool.query(
    `UPDATE promotions SET
     name=$1, description=$2, type=$3, value=$4, buy_qty=$5, get_qty=$6, min_purchase=$7,
     scope=$8, scope_ids=$9::jsonb, product_id=$10, category=$11,
     start_date=$12, end_date=$13, start_time=$14, end_time=$15, days_of_week=$16::jsonb,
     priority=$17, max_uses=$18, stackable=$19, active=$20, gift_product_id=$21,
     code=$22, requires_code=$23
    ...
    [n.name, n.description, n.type, n.value, n.buy_qty, n.get_qty, n.min_purchase, n.scope, JSON.stringify(n.scope_ids),
     ...
     n.priority, n.max_uses, n.stackable, n.active, n.gift_product_id, n.code, n.requires_code, id]
  );
  if (result.rows.length === 0) return fail(404, 'Promotion not found');
  return ok(result.rows[0]);
});

export const DELETE = handle(async (_request, { params }) => {
  await pool.query('DELETE FROM promotions WHERE id = $1', [params.id]);
