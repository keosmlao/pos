...
const fmtNum = n =>
  new Intl.NumberFormat('lo-LA').format(Math.round(Number(n) || 0));
...
function labelDims(layoutKey) {
  // ✅ Fixed custom label size
  if (layoutKey === '100x35.5') {
    return {
      lw: 100,
      lh: 35.5,
      cols: 2,
  ...
  return (
    <div className="space-y-4 pb-6">
      <AdminHero
        tag="Price labels"
        title="🏷 ພິມປ້າຍລາຄາ"
        subtitle="ເລືອກສິນຄ້າ + ຈຳນວນ → ກຳນົດຂະໜາດ → ພິມ"
        action={
          <button
            onClick={print}
        ...
        metrics={[
          {
            label: 'ສິນຄ້າທີ່ເລືອກ',
            value: fmtNum(selectedItems.length),
          },
          {
            label: 'ປ້າຍລວມ',
            value: fmtNum(totalLabels),
            tone: 'emerald',
          },
          {
            label: 'ຮູບແບບ',
            value: LAYOUTS[layoutKey].label,
          },
          {
            label: 'ຂະໜາດ/ປ້າຍ',
            value: (() => {
              const d = labelDims(layoutKey);
              return `${d.lw.toFixed(1)}×${d.lh.toFixed(1)}mm`;
            })(),
          },
        ]}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4">
        <div className="space-y-3">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
            <h2 className="font-bold text-slate-900">ຮູບແບບປ້າຍ</h2>

            <div className="grid grid-cols-2 gap-2">
              {['4', '6', '8', '1', '100x35.5'].map(k => {
                const L = LAYOUTS[k];
                const d = labelDims(k);
                ...
                >
                  ...
                  <div
                    className={`text-xs font-extrabold mt-0.5 ${
                      orientation === k ? 'text-red-700' : 'text-slate-800'
                    }`}
                  >
                    {v.label}
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selectedItems.length > 0 && (
                <button
                  onClick={clearAll}
                  className="px-3 py-1.5 text-rose-600 hover:bg-rose-50 rounded-md text-xs font-bold"
                >
                  ລ້າງ
                </button>
              )}
              ...
              <table className="w-full text-xs">
                <thead className="bg-slate-50 sticky top-0">
                  <tr className="text-left">
                    ...
                    <th className="px-3 py-2 font-bold text-slate-600 text-right">
                      ລາຄາ
                    </th>
                    <th className="px-3 py-2 font-bold text-slate-600 text-right w-24">
                      ຈຳນວນ
                    </th>
                  </tr>
                </thead>
                        ...
                        >
                          <td className="px-3 py-1.5">
                            <input
                              ...
                              checked={qty > 0}
                              onChange={e =>
                                updateQty(p.id, e.target.checked ? 1 : 0)
                              }
                              className="accent-red-600"
                            />
                          </td>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-xs text-slate-500">
            {selectedItems.length === 0
              ? 'ເລືອກສິນຄ້າເພື່ອເບິ່ງຕົວຢ່າງ'
              : `${fmtNum(totalLabels)} ປ້າຍ`}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="text-xs text-slate-500 text-center">
        {cols}×{rows} = {perPage} ປ້າຍ/A4 · ປ້າຍ {lw.toFixed(1)}×
        {lh.toFixed(1)}mm
...
function MiniLabel({ product: p, company, opts, isLandscape, scale, lw, lh }) {
  ...
  return (
    <div
      ...
      className={`bg-white border border-dashed border-slate-300 flex overflow-hidden ${
        isLandscape
    ...
    >
      {isLandscape && opts.show_image && (
        <div
          className="rounded bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden shrink-0 self-center"
          style={{
            width: `${imgSize * scale * 1.3}px`,
            height: `${imgSize * scale * 1.3}px`,
          }}
        >
          {p.image_url ? (
            ...
            <span
              style={{ fontSize: `${baseFont * scale * 1.5}px` }}
      ...
      <div
        className={`flex-1 flex flex-col min-w-0 ${
          isLandscape ? 'items-start text-left' : 'items-center justify-between w-full'
        }`}
        style={{ gap: `${0.5 * scale}px` }}
      >
        {opts.show_shop && (
          <div
            className="font-bold text-slate-700 truncate w-full border-b border-dotted border-slate-300"
            style={{
              fontSize: `${7 * scale}px`,
              paddingBottom: `${0.5 * scale}px`,
              marginBottom: `${0.5 * scale}px`,
        ...
        >
          {!isLandscape && opts.show_image && (
            <div
              className="rounded bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden"
              style={{
                width: `${imgSize * scale}px`,
                height: `${imgSize * scale}px`,
              }}
            >
              {p.image_url ? (
                <img src={p.image_url} alt="" className="w-full h-full object-contain" />
                ...
                <span
                  style={{ fontSize: `${baseFont * scale}px` }}
                  className="text-slate-300"
                >
                  📦
                </span>
          ...
          <div
            className="font-extrabold text-slate-900 leading-tight w-full truncate"
            style={{
          ...
          <div
            className="font-extrabold text-red-600 leading-none"
            style={{ fontSize: `${priceFont * scale}px` }}
          >
            {new Intl.NumberFormat('lo-LA').format(
              Math.round(Number(p.selling_price) || 0)
            )}

            {opts.show_currency && (
              <span
                style={{
                  fontSize: `${priceFont * 0.5 * scale}px`,
        ...
        {opts.show_barcode && p.barcode && (
          <div
            className="w-full border-t border-dotted border-slate-300"
            style={{
              paddingTop: `${0.5 * scale}px`,
              marginTop: `${0.5 * scale}px`,
              textAlign: isLandscape ? 'left' : 'center',
            }}
          >
            <div
...
function buildLabelsHtml({
  ...
  const nameFont =
    area >= 9000 ? 14 : area >= 5000 ? 11 : area >= 3000 ? 9 : 7;

  const priceFont =
    area >= 9000 ? 32 : area >= 5000 ? 24 : area >= 3000 ? 18 : 14;

  const imgSize = Math.max(12, Math.min(34, Math.min(lw, lh) * 0.4));
    ...
    const info = `
      <div class="info">
        ...
        <div class="main">
          ${
            !isLandscape && imageEl
              ? `<span class="in-main">${imageEl.replace(
                  'class="img"',
                  'class="img in-main"'
                )}</span>`
              : ''
          ...
          ${
            opts.show_code && p.product_code
              ? `<div class="code">${esc(p.product_code)}</div>`
              : ''
          }

          <div class="price">
            ${fmtNum(p.selling_price)}
            ${opts.show_currency ? '<span class="cur">₭</span>' : ''}
            ${
              opts.show_unit && p.unit
        ...
        ${
          opts.show_barcode && p.barcode
            ? `<div class="bc-num">${esc(p.barcode)}</div>`
            : ''
        }
      </div>
    `;
  ...
  const gridStyle = `
    .sheet {
      display: grid;
      grid-template-columns: repeat(${d.cols}, ${lw}mm);
      grid-template-rows: repeat(${d.rows}, ${lh}mm);
      gap: ${GAP}mm;
      width: ${PAGE_W - MARGIN * 2}mm;
      min-height: ${PAGE_H - MARGIN * 2}mm;
...
<style>
  ...
  * {
  ...
  .label {
    ...
    ${
      isLandscape
        ? 'flex-direction: row; align-items: stretch; gap: 1.5mm;'
  ...
  ${
    isLandscape
      ? `
  .label > .img:not(.in-main) {
    width: ${imgSize * 1.4}mm;
    height: ${imgSize * 1.4}mm;
    align-self: center;
    flex-shrink: 0;
  }
  ...
  .shop {
    font-size: ${Math.max(5, nameFont * 0.55)}pt;
    font-weight: 700;
    color: #475569;
    width: 100%;
    ${isLandscape ? '' : 'text-align: center;'}
  ...
  .name {
    ...
    ${isLandscape ? 'text-align: left;' : 'text-align: center;'}
    display: -webkit-box;
    -webkit-line-clamp: 2;
  ...
  .code {
    ...
    font-size: ${Math.max(5, nameFont * 0.7)}pt;
    color: #64748b;
  }

  .price {
    font-size: ${priceFont}pt;
    font-weight: 900;
    color: #b91c1c;
    line-height: 1;
    ${isLandscape ? 'text-align: left; width: 100%;' : ''}
  }

  .cur {
    font-size: ${priceFont * 0.5}pt;
    margin-left: 1mm;
  }
  ...
  .bc {
    width: 100%;
    max-height: ${Math.min(12, lh * 0.22)}mm;
    margin-top: 0.5mm;
    flex-shrink: 0;
  }
...
<body>
  ${sheets
    .map(
      pageLabels => `
    <div class="sheet">
      ${pageLabels.map((p, i) => labelEl(p, i)).join('')}
    </div>
  `
    )
    .join('')}
